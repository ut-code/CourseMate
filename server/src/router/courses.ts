...
const router = new Hono()
  ...
  .get(
    ...
    async (c) => {
      ...
      const uniqueFilteredOverlappingCourses =
        filteredOverlappingCourses.filter(
          (course, index, self) =>
  ...
  // 自分の講義を編集
  .patch("/mine", json(z.object({ courseId: z.string() })), async (c) => {
    const userId = await getUserId(c);
  ...
  // 自分の講義を削除
  .delete("/mine", json(z.object({ courseId: z.string() })), async (c) => {
    const userId = await getUserId(c);
    const { courseId } = c.req.valid("json");
