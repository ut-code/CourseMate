...
test("recommendation engine", async () => {
  ...
  expect(usersFor101.map((entry) => entry.u.id)).toEqual([102, 103]);

  const usersFor102 = await recommendedTo(102, 5, 0);
  expect(usersFor102.map((entry) => entry.u.id)).toEqual([103, 101]);

  const usersFor103 = await recommendedTo(103, 5, 0);
  expect(usersFor103.map((entry) => entry.u.id)).toEqual([102, 101]);
});
