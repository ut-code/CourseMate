...
// ImageData 形式の画像をfirestoreにアップロードする関数
export async function uploadImageData(image: ImageData): Promise<string> {
  const guid = getAuth().currentUser?.uid;
  if (!guid) throw new Error("Failed to get user");

  const storage = getStorage(app);
  const filePath = `${guid}/${name}`;
  const storageRef = ref(storage, filePath);

  try {
    const snapshot = await uploadBytes(storageRef, image.data);
    const pictureUrl = await getDownloadURL(snapshot.ref);
    console.log("File available at", pictureUrl);
    return pictureUrl;
  } catch (error) {
